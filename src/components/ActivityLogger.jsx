...
import {
  calcTransportEmission, calcFoodEmission,
  calcEnergyEmission, calcShoppingEmission, formatEmission,
...
export default function ActivityLogger({ onAdd, error }) {
  ...
  if (!isNaN(amountNum) && amountNum > 0) {
    try { previewKg = config.calc(activityType, amountNum); } catch { previewKg = null; }
  }
  ...
  function handleSubmit(e) {
    ...
    const success = onAdd({ category, activityType, amount: amountNum, emissionKg, notes: notes.slice(0, 200) });
    if (success !== false) {
      setAmount('');
      setNotes('');
      setSubmitted(true);
  ...
  return (
    <section className="card logger-card" aria-label="Log a new activity">
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="field">
          <label htmlFor="activity-type" className="field-label">Activity type</label>
          <div className="select-wrap">
            <select id="activity-type" value={activityType}
              onChange={(e) => setActivityType(e.target.value)} className="field-select">
        ...
        <div className="field">
          <label htmlFor="activity-amount" className="field-label">{config.unitLabel}</label>
          <input id="activity-amount" type="number" min="0" step="any"
            value={amount} onChange={(e) => setAmount(e.target.value)}
            className="field-input" placeholder="Enter amount"
            required aria-describedby="amount-preview" />
        ...
        <div className="field">
          ...
          <input id="activity-notes" type="text" value={notes}
            onChange={(e) => setNotes(e.target.value)}
            className="field-input" placeholder="e.g. commute to work" maxLength={200} />
        </div>
