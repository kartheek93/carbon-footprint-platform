...
export default function Dashboard({ entries, totalKg, byCategory, annualProjection, streak, earnedBadges }) {
  ...
  return (
    <div className="dashboard">
      <div className="stats-grid" role="region" aria-label="Carbon footprint summary">
        <StatCard icon={Leaf} label="Total logged" value={formatEmission(totalKg)}
          sub={`${entries.length} activit${entries.length === 1 ? 'y' : 'ies'}`} color={emissionColor} />
        <StatCard icon={Target} label="Annual projection" value={formatEmission(annualProjection)}
          sub="estimated / year" color={emissionColor} />
        <StatCard icon={streak > 0 ? TrendingDown : TrendingUp} label="Logging streak"
          value={`${streak} day${streak !== 1 ? 's' : ''}`} sub="consecutive days" color="#f59e0b" />
        <StatCard icon={Award} label="Badges earned" value={earnedBadges.length}
          sub="achievements" color="#8b5cf6" />
      </div>

      <section className="card" aria-label="Emissions benchmarks">
        <h2 className="section-title">How you compare</h2>
        <div className="benchmarks">
          <BenchmarkBar label="Your projection" value={annualProjection || 0} max={maxBar} color={emissionColor} />
          <BenchmarkBar label="Global average" value={GLOBAL_AVERAGE_KG_PER_YEAR} max={maxBar} color="#f97316" />
          <BenchmarkBar label="Paris target" value={TARGET_KG_PER_YEAR} max={maxBar} color="#4ade80" />
        </div>
        {annualProjection > 0 && (
      ...
      <div className="charts-grid">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={trendData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ background: '#1e2d23', border: '1px solid #2d4a36', borderRadius: 8 }}
        ...
        <section className="card" aria-label="Emissions by category">
          ...
          {categoryData.length > 0 ? (
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie cx="50%" cy="50%" innerRadius={45} outerRadius={75}
                  paddingAngle={3} dataKey="kg" data={categoryData}
